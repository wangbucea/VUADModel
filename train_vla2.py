...
class VLATrainer:
    ...
    def prepare_targets(self, batch):
        ...
        # 动作序列目标 - 使用slave_actions作为DIT模型的目标
        if 'slave_actions' in batch:
            targets['slave_actions'] = batch['slave_actions'].to(self.device)  # [B, seq_len, 7]
        
        # 保留master_actions用于其他任务
    ...
    def train_epoch(self):
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            outputs = self.model(
                ...
                target_actions=targets.get('master_actions')
            )
