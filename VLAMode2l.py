...
class DualViewDINOv2VisualEncoder(nn.Module):
    """双视角DINOv2视觉编码器"""
    def __init__(self, model_name="facebook/dinov2-base", freeze_backbone=False, image_size=(480, 640)):
        super().__init__()
...
class PixelTransformerDecoder(nn.Module):
    ...
    def __init__(self, feature_dim=256, num_classes=10, image_size=(480, 640)):
        super().__init__()
        self.feature_dim = feature_dim
...
class RobotUniADModel(nn.Module):
    ...
    def forward(self, images, input_actions=None, target_actions=None, det_targets=None):
        ...
        if self.training and target_actions is not None:
            ...
            if input_actions is not None:
                # input_actions现在是[B, action_dim]，直接使用
    ...
    def compute_loss(self, outputs, targets):
        ...
        if 'seg_masks' in targets:
            ...
            seg_logits_adjusted = outputs['seg_logits']  # [B, C, H, W]
        ...
        if 'slave_actions' in targets:
            if self.training and hasattr(self, '_training_noise'):
                ...
                trajectory_loss = F.mse_loss(
                    predicted_actions, 
                    targets['master_actions']  # 使用slave_actions作为目标
                )
...
if __name__ == "__main__":
    ...
    batch_size = 2
    timesteps = 2
    images = torch.randn(batch_size, timesteps, 3, 480, 640)
